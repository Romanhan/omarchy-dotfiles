...
Item {
  ...
  // NOTE: plugin bars are loaded via Loader.source (no context bindings), and a
  // root `required property` makes that load fail. Plain properties are set by
  // the host's configureBar() right after load, like plugin panels.
  ...
  // Dynamic island: when false only the core widgets are shown and the bar
  // shrinks to a floating pill; revealing mounts the hidden layout widgets.
  property bool islandRevealed: false
  // Widgets always visible on the island regardless of reveal state.
  property var islandCoreIds: [
    "omarchy.workspaces", "romanhan.clock", "romanhan.audio",
    "romanhan.network", "romanhan.power", "island-reveal",
    "omarchy.system-update", "omarchy.indicators"
  ]
  // Vertical offset of the floating island from the screen edge.
  property int islandGap: 3
  // Gap between the pill and the windows below it. Standalone value; 0 = pill
  // sits flush against the windows.
  property int barGap: 0
  // Height of the floating island pill; overrides Style.bar.sizeHorizontal.
  property int islandHeight: 30
  // Horizontal gap between modules inside the island pill.
  property int islandModuleGap: Style.space(150)
  function toggleIslandReveal(): void { islandRevealed = !islandRevealed }
  IpcHandler {
    target: "romanhan.bar"
    function toggleReveal(): string {
      root.toggleIslandReveal()
      return "ok"
    }
  }
  ...
  // Number of popups holding the squared-corner look (open through their close
  // animation). The corner rounds back only when this reaches zero, so the bar
  // stays merged with the popup for the entire open+close motion.
  ...
  readonly property int barSize: vertical ? Style.bar.sizeVertical : islandHeight
  ...
  function layoutEntries(region) {
    ...
    if (!root.islandRevealed) {
      var core = root.islandCoreIds
      entries = entries.filter(function(entry) {
        return core.indexOf(root.entryId(entry)) !== -1
      })
    }
    return entries
  }

  // Unfiltered layout for a region. The island mounts every entry at all
  // times and gates each slot's width/opacity on reveal state, so the pill
  // morphs in lock-step with the content instead of lagging behind it.
  function fullLayoutEntries(region) {
    var entries = layoutConfig ? layoutConfig[region] : null
  ...
  component BarPanel: PanelWindow {
    ...
    // Width of the island content (module row + pill padding), reported by the
    // horizontal content so the layer surface can shrink/grow to hug it.
    readonly property real islandContentWidth: barContent.item && barContent.item.contentWidth
        ? barContent.item.contentWidth : 0
    ...
    // Full-width transparent surface + capsule mask: the layer surface never
    // resizes during a morph (so its straight edges can't clip the rounded
    // ends), and input/visibility is limited to the animated capsule.
    mask: Region {
      x: root.vertical ? 0
        : Math.floor(barContent.item ? barContent.item.pillX : 0) - 2
      y: root.vertical ? 0
        : Math.floor(barContent.item ? barContent.item.pillY : 0) - 2
      width: root.vertical ? barWindow.width
        : Math.ceil(barContent.item ? barContent.item.pillW : 0) + 4
      height: root.vertical ? barWindow.height
        : Math.ceil(barContent.item ? barContent.item.pillH : 0) + 4
    }
    ...
    margins {
      top: root.barHidden && root.position === "top" ? -root.barSize
          : root.vertical ? 0 : root.islandGap - 2
    ...
    // +4 gives the pill 2px clearance from the surface edges on each side, so
    // fractional-scaling pixel snapping can never shave the rounded arcs.
    implicitHeight: root.vertical ? 0 : root.barSize + 2 * root.barGap
    color: "transparent"
...
color: root.vertical ? (root.transparent ? "transparent" : root.background) : "transparent"
    ...
    Component {
      ...
      Item {
        ...
        // The horizontal island: a centered pill that animates its width as
        // hidden widgets are revealed. The content row fills the pill, with
        // flexible side padding that keeps the module group centered and
        // absorbs any tiny frame lag during the morph so the rounded ends
        // stay clean and content never appears clipped.
          readonly property real islandPad: root.barSize / 2 - 5
        readonly property real contentWidth: islandRow.implicitWidth
        // Exposed so the PanelWindow mask can track the pill as it animates.
        readonly property real pillX: islandPill.x
        readonly property real pillY: islandPill.y
        readonly property real pillW: islandPill.width
        readonly property real pillH: islandPill.height

        BorderSurface {
          id: islandPill
          anchors.centerIn: parent
          width: islandRoot.contentWidth + 4
          height: root.barSize
          radius: root.barSize / 2
          color: root.transparent ? "transparent" : root.background
          borderSpec: Border.flat(root.barForeground, 0)
          ...
          RowLayout {
            id: islandRow
            anchors.centerIn: parent
            width: islandRoot.contentWidth
            height: parent.height
            spacing: 0

            Item {
              implicitWidth: islandPad
              width: islandPad
              implicitHeight: 1
              Layout.preferredWidth: islandPad
              Layout.fillWidth: false
            }
            ModuleList {
              region: "left"
              entries: root.fullLayoutEntries("left")
              Layout.preferredWidth: item ? item.implicitWidth : 0
            }
            Item {
              implicitWidth: root.islandModuleGap
              implicitHeight: 1
            }
            ModuleList {
              region: "center"
              entries: root.fullLayoutEntries("center")
              Layout.preferredWidth: item ? item.implicitWidth : 0
            }
            Item {
              implicitWidth: root.islandModuleGap
            ...
            ModuleList {
              region: "right"
              entries: root.fullLayoutEntries("right")
              Layout.preferredWidth: item ? item.implicitWidth : 0
            }
Item {
              implicitWidth: islandPad
  ...
  component ModuleSlot: Item {
    ...
    readonly property bool slotRevealed: root.islandRevealed || root.islandCoreIds.indexOf(moduleName) !== -1
    ...
    // Tide-island style synchronized reveal: the slot box animates in lockstep
    // with the pill's own width Behavior, so content never overflows the rounded
    // capsule (which produced the straight clip line on reveal / content change).
    readonly property real slotTargetWidth: slotRevealed
      ? (activeItem && activeItem.visible
          ? (root.vertical ? root.barSize : activeItem.implicitWidth)
          : (activeItem && slot.moduleName === "omarchy.weather" ? activeItem.implicitWidth : 0))
      : 0
    readonly property real slotTargetHeight: slotRevealed && activeItem && activeItem.visible ? activeItem.implicitHeight : 0
    property real slotAnimWidth: slotTargetWidth
    property real slotAnimHeight: slotTargetHeight
    Behavior on slotAnimWidth {
      NumberAnimation { duration: 280; easing.type: Easing.OutQuint }
    }
    Behavior on slotAnimHeight {
      NumberAnimation { duration: 280; easing.type: Easing.OutQuint }
    }
    implicitWidth: slotAnimWidth
    implicitHeight: slotAnimHeight
    width: slotAnimWidth
    height: slotAnimHeight
